...
from app.graphql.corpus_recommendation import CorpusRecommendation, NewTabCorpusRecommendation
...
def new_tab_corpus_slate_recommendations_resolver(
        root: 'NewTabCorpusSlate', count: Optional[int] = DEFAULT_RECOMMENDATION_COUNT
) -> List[NewTabCorpusRecommendation]:
    """
    Currently, the only way to define arguments on nested objects in Strawberry is to define a resolver.
    :param root: The CorpusSlate object of which recommendations is a field.
    :param count: The maximum number of Corpus recommendations to return
